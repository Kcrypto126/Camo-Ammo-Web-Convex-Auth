...
export const generateUploadUrl = mutation({
  args: {},
  handler: async (ctx) => {
    console.log("[generateUploadUrl] Starting upload URL generation");
    const user = await getCurrentUser(ctx);
    if (!user) {
      console.log("[generateUploadUrl] User not found or not authenticated");
      throw new ConvexError({
        message: "User not logged in",
        code: "UNAUTHENTICATED",
      });
    }
    console.log("[generateUploadUrl] User found:", user._id);
